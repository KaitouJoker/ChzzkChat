...
- **⚙️ 간편한 설정 파일 지원 (`config.json`)**:
  - 스트리머 ID, 저장 경로, 확인 주기 등을 `config.json`에 저장하여 매번 인자를 입력할 필요 없이 `python run.py`만으로 실행할 수 있습니다.
...
| `output_csv` | `chat.csv` | 채팅/후원 데이터가 누적 저장될 CSV 파일 경로 |
...
저장되는 `chat.csv` 파일은 다음과 같은 구조로 실시간 기록됩니다:
